...
# Remove time from datetime in event_time column
...
df_connections_count = df_connect.groupby(['map', 'event_time', 'server_id']).size().reset_index(name='connections_count')

# COMMAND ----------

# Find number of disconnections witht the unique combinations those three columns
df_disconnections_count = df_disconnect.groupby(['map', 'event_time', 'server_id']).size().reset_index(name='disconnections_count')
...
merged_df_connections = pd.merge(df_connections_count, df_disconnections_count, on=['event_time', 'map', 'server_id'], how='outer')
